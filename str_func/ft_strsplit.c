...
static int		ft_test_and_set(int *i, int *index, char const *s)
{
	if (!s)
		return (0);
	*i = 0;
...
char			**ft_strsplit(char const *s, char c)
{
	...
	if (ft_test_and_set(&i, &index_w, s) == 0)
		return (NULL);
	if (!(tab = (char **)malloc(sizeof(char *) * (ft_nbrwords(s, c) + 1))))
		return (NULL);
	if (ft_nbrwords(s, c) == 0)
	...
	while (s[i] != '\0')
	{
		...
		if (s[i] != '\0')
		{
			if (!(tab[index_w] = ft_newword((char*)(&s[i]), c)))
